...
<div class="meta-row">
    ...
    <span>Total Questions: <strong>{{ $exam->questions->count() }}</strong></span>
        <span>MCQ pass: <strong>{{ $exam->mcqPassPercentage() }}%</strong></span>
        <span>Writing pass: <strong>{{ $exam->writingPassPercentage() }}%</strong></span>
            ...
            <tr>
                ...
                <td>
                    <span class="badge badge-score">
                        @if($ready)
                            {{ number_format($s['obtained'], 2) }} / {{ number_format($s['total'], 2) }}
                        @else
                            MCQ {{ number_format($s['mcq_obtained'], 2) }}/{{ number_format($s['mcq_total'], 2) }} · Writing pending
                        @endif
                    </span>
                </td>
...
<div class="footer">
    ...
    <span>Generated by {{ config('brand.name') }} on {{ now()->format('M d, Y H:i:s') }}</span>
</div>
